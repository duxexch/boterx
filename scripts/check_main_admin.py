...
def main():
    ...
    try:
        ...
        print("\n=== CHECK MAIN DASHBOARD ===")
        run(ssh, "curl -s -b /tmp/cookies.txt -w 'dashboard:%{http_code} time:%{time_total}\n' -o /dev/null 'http://127.0.0.1:8080/dashboard'")

        # Check dashboard content
        run(ssh, "curl -s -b /tmp/cookies.txt 'http://127.0.0.1:8080/dashboard' | head -c 5000 > /tmp/dash.html; cat /tmp/dash.html")
        ...
        # Check main admin template
        run(ssh, "curl -s -b /tmp/cookies.txt 'http://127.0.0.1:8080/dashboard' | head -c 5000")

        # Check base.html for language/theme toggles
