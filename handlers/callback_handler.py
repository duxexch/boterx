    ...
    def handle_callback_query(self, callback):
        ...
        try:
            ...
            # ==================== مطابقة: موافقة/رفض الأدمن على الطلب ====================
                ...
                # رفض ذرّي على SQLite: يلغي معاملة الوكيل المعلقة ويحرر escrow
                rejected_user_id = ''
                try:
                    import agent_db as _adb
                    _full = _adb.get_match_request_full(req_id)
                    if _full:
                        rejected_user_id = str(_full.get('user_id', ''))
                    ok, err = _adb.admin_set_match_request_status(
                        req_id, 'rejected', actor=str(user_id))
                    if not ok and not rejected_user_id:
                        self.edit_message(chat_id, message.get('message_id'), f"⚠️ {err}")
                        return
                except Exception as e:
                    logger.error(f"match admin reject error: {e}")
                self.edit_message(chat_id, message.get('message_id'), self.tr('a0494_تم_رفض', 'ar'))
                # إشعار العميل
                if rejected_user_id:
                    try:
            ...
            elif data == 'match_admin_refresh':
                # تحديث لوحة المطابقات بأحدث البيانات
                fake_msg = {'chat': {'id': chat_id}, 'from': {'id': user_id}, 'text': ''}
                try:
