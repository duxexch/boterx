...
class MessageDispatcherMixin:
    ...
    def process_message(self, message):
        ...
        _all_langs_reset = set(self._tset('reset_system'))
        _all_langs_reset |= set(self._tset('main_menu'))
        ...
        cancel_texts = set(self._tset('cancel_btn')) | set(self._tset('cancel_registration')) | {'❌ إلغاء', '❌ Cancel', 'إلغاء', 'الغاء', 'الغاء'}
        main_menu_texts = set(self._tset('main_menu')) | set(self._tset('main_menu_btn')) | {'🏠 القائمة الرئيسية', '🏠 الرئيسية', '🏠', '🏠 Main Menu'}
        back_texts = set(self._tset('back_btn')) | set(self._tset('back_to_main')) | {'🔙', '🔙 Back', '🔙 رجوع', '🔙 العودة'}
        ...
        all_langs = self.get_supported_languages()
        skip_texts = set(self._tset('skip_registration'))
        register_texts_new = set(self._tset('register_account'))
        register_texts_new.add(self.tr('a0218_تسجيل_حساب', 'ar'))
        reset_texts = set(self._tset('reset_system'))
        ...
        if self.is_admin(user_id):
            admin_texts = set(self._tset('admin_panel_btn')) | {'🔧 Admin', self.tr('a0223_لوحة_الإدارة', 'ar')}
        ...
        deposit_texts = set(self._tset('deposit'))
        withdraw_texts = set(self._tset('withdraw'))
        requests_texts = set(self._tset('my_requests'))
        profile_texts = set(self._tset('profile'))
        complaint_texts = set(self._tset('complaint'))
        support_texts = set(self._tset('support'))
        currency_texts = set(self._tset('change_currency'))
        match_texts = set(self._tset('match_btn')) | {self.tr('a0227_مطابقة', user_lang), '🔄 Match'}
        notif_texts = set(self._tset('notif_btn')) | {self.tr('a0228_إشعاراتي', user_lang), '🔔 Notifications'}
        ref_texts = set(self._tset('referral_btn')) | {self.tr('a0229_اربح', user_lang), self.tr('a0230_إحالة', user_lang), '🎁 Referral'}
        help_texts = set(self._tset('help_btn_label')) | {self.tr('a0231_مساعدة', user_lang), '❓ Help'}
        svrp_texts = set(self._tset('svrp_title')) | {self.tr('a0232_تعويض', user_lang), self.tr('a0233_تعويض', user_lang)}
        apps_texts = set(self._tset('apps_btn')) | {self.tr('a0117_تطبيقات', user_lang), '📱 Apps'}
        # مطابقة ديناميكية لأزرار اليانصيب والعجلة والمحفظة والمزيد — كل الترجمات + النصوص القديمة
        lottery_texts = set(self._tset('a0235_يانصيب')) | {'🎰 يانصيب', '🎰 Lottery'}
        wheel_texts = set(self._tset('a0236_عجلة_الحظ')) | {'🎡 عجلة الحظ', '🎯 صيد الجوائز', '🎁 اختطف', '🎡 Wheel'}
        wallet_texts = set(self._tset('a0237_محفظتي')) | {'💎 محفظتي', '💎 Wallet'}
        more_texts = set(self._tset('a0238_المزيد')) | {'⚙️ المزيد', '⚙️ More'}
        register_texts = set(self._tset('register_account'))
        ...
        if text in deposit_texts:
            ...
            all_langs = self.get_supported_languages()
            svrp_wallet_texts = set(self._tset('svrp_my_wallet'))
            svrp_tasks_texts = set(self._tset('svrp_my_tasks'))
            svrp_codes_texts = set(self._tset('svrp_my_codes'))
            svrp_create_texts = set(self._tset('svrp_create_code_btn'))
            svrp_redeem_texts = set(self._tset('svrp_redeem_code_btn'))
            svrp_tree_texts = set(self._tset('svrp_referral_tree_btn'))
            svrp_tier_texts = set(self._tset('svrp_my_tier_btn'))
    ...
    def handle_start_phone(self, message):
        ...
        if 'contact' in message:
            ...
            if phone in set(self._tset('enter_phone_manual')):
                self.send_message(chat_id,
                    f"✍️ {self.tr('enter_phone_prompt', selected_lang)}")
                return

            # إذا ضغط زر القائمة الرئيسية
            if phone in [self.tr('a0083_القائمة_الرئيسية', 'ar'), '🏠'] or phone in set(self._tset('main_menu_btn')):
                if user_id in self.user_states:
                    del self.user_states[user_id]
                self.handle_start(message)
                return
    ...
    def handle_phone_login(self, message):
        ...
        text = message.get('text', '').strip()
        all_langs = self.get_supported_languages()
        main_menu_texts = set(self._tset('main_menu')) | set(self._tset('main_menu_btn')) | {self.tr('a0083_القائمة_الرئيسية', lang), self.tr('a0141_الرئيسية', lang), '🏠 Main Menu'}
